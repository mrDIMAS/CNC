...
line_t line_queue[ LQ_SIZE ];

volatile uint8_t line_queue_head = 0;
...
void exec_string( char * str )
{
	...
	while( !commandDone ) { }; // wait until command becomes done
    ...
    while( 1 )
    {
	    ...
	    if( !busy )
	    {
		    ...
		    {	
				...
				exec_string( commandBuffer );											
				/*
				exec_string( "L 0 0 200 300\0");
				exec_string( "L 200 300 400 0\0");
				exec_string( "L 400 0 0 200\0");
				exec_string( "L 0 200 400 200\0");
				exec_string( "L 400 200 0 0\0");	*/
					/*				
				lq_add_line(   0,   0, 200, 300 );
